 ...
 * 输出 token 数组而非 HTML 字符串：渲染端按 token 类型结构化输出，不经 v-html / innerHTML，
 * 天然免疫 XSS（URL 走协议白名单校验）。支持链接/图片/粗体/斜体/行内代码/删除线，按行切分段落。
 * 未加语法的 http(s) 裸链接自动识别为可点击链接；无任何标记时整段按纯文本渲染。
 */
