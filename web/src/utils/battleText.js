/**
 * 战斗文本结构化解析器：把服务端下发的战斗结算纯文本（攻击/炮击/地图战斗回合的结果）
 * 解析成结构化段落，交给 BattleCard.vue 渲染成带动画的战斗卡片。
 * - 仅做网页端展示层增强：只读原文做结构化展示，绝不改写原文；解析失败回退为原始文本直出，
 *   后端与 AstrBot（QQ 等）始终走标准纯文本消息。
 * - 文本特征全部来自 server/src/modules/game/combat-system.service.ts 的 resultLines 拼接规则，
 ...
 *   - 闪避状态行：`你处于闪避状态（剩余N秒），闪开了攻击`
 *   - 玩家攻击未命中简版行：`怪物名 闪避了攻击`
 *   - 模板闪避/未命中行：`…躲开了…`（attackText 模板，如"【目标】躲开了【名称】的拳头"，
 *     开启显示倍率后尾部还会带 `(倍率N%)`）
 *   - 光荣弹：`…引爆【光荣弹】，对 …造成 …`
