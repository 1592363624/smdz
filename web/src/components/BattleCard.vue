<!--
 * BattleCard.vue：把服务端下发的「战斗结算纯文本」（攻击/炮击/地图战斗回合）渲染成
 * 「修真 × 科幻」风格的动画卡片（伤害重击砸落、暴击迸发、击杀烙印、掉落芯片、受击震屏）。
 * 展示层增强：只读原文不改写内容，后端与 AstrBot（QQ 端）仍走标准纯文本；解析失败回退原文直出。
 * 解析逻辑在 utils/battleText.js（纯函数，正则与服务端战斗文案一一对应）。
 * 动画全为 CSS 一次性入场（animation-fill-mode: both），用 --d 逐行错峰，避免多卡片同时循环动画；
