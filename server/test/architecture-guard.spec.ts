...
describe('架构门禁：玩家状态写入口收口', () => {
  ...
  // 基线 17,434 → 17,281（2026-09-12 P1 批次）→ 16,967（P2-1 ranking 批次）→ 16659（P2-2 admin 批次）→ 15221（P2-3~P2-5 批次）：P1-1 消重（roundText/displayDamage）+
  // P1-2 时长辅助收敛 + P1-3 支撑层 22 方法迁出 game-support.service.ts，按实测下调。
  const GAME_SERVICE_LINE_BASELINE = 15221;
