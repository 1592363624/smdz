...
for (let i = 0; i < lines.length; i++) {
  ...
  // 只排除箭头函数属性行与注释行：不能按 `includes('=')` 过滤，否则带默认参数值的
  // 方法声明（如 `handleSweep(userId, requestedCount = 0)`）会被误杀，
  // 导致其方法体并入前一个方法、调用边错归属。
...
const G = {
  ...
  shop: ['withTradeLock','handleTrade','handleHomeTrade','handleShop','handleActivityShop','handleDiamondShop','handleDataShop','handleAutoShop','handleGive','findMerchantInSummons','checkMerchantPurchaseGate','formatMerchantItem','formatMerchantItems','hasEnoughResources','generateMerchantResource','generateMerchantInventory','buildMerchantInventory','purchaseMerchantItem','handleRecipe','handleRecipeUnlock','handleReverse','readReverseProficiencies','reverseProficiency','setReverseProficiency','reverseValue','reverseItemFailure','reverseOne','reverseAllEligible','formatReverseMenu','formatReverseBatchResult','formatReverseSingleResult','saveReverseResult'],
  ...
  delayed: ['onModuleInit','recoverOrphanDelayedMarkers','handleReload','scheduleReloadCompletion','completeReload','handleRefill','completeRefill','handleSaveImage','handleStartSaveImage','handleStopSaveImage'], // 注册块与启动迁移永久留门面
  inventory: ['handleInventory','getBackpackDisplayItems','handleResourceBag','handleSearchBag','handleSearchSafe','handleBagOps','handleUseItem','handleUseAllItems','getSeedCropName','handleUseSeed','handleViewDescription','handleViewMarkers','handleViewMarkers2','backpackQuantity','addBackpackItem'], // getBackpackDisplayItems 属支撑层
  ...
  facade: ['constructor'], // 门面构造器：dts.registerHandler 延时接线 + 启动迁移，永久留门面
  combat: ['handleAttack'], // 战斗域：已知留白，L0 无环叶子
  achievement: ['handleViewAchievements'], // 成就域：单方法叶子
...
const SHARED_CANDIDATES = ['advanceTask','mutatePlayer','pushVehicleParts','buildNumberedMenu','incrementMarker','normalizeMarkers2','hasEquip','setMarkers2','itemName','itemType','itemQuantity','deductBackpackItem','addItemToCollection','addItemToCollection','getPlayerName','getCurrentMap','round2Text','millisecondsToText','formatUptime','randomInt','parseJsonArray','firstPositiveNumber','secondsToTimeText','formatGatherNumber','formatReverseNumber','resolveGatherCmd','updateOwnedSummonMode','hasTrainerAccess','hasEquippedSpecial','equipmentSpecialSeq','currentWeaponItem'];
