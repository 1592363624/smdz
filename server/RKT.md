...
| _初始化怪物(深层) | L2748-3052 | `map.service`/`combat-state` | 🔶 | **基石层已建**(同上)。待补：装备生成(L2748)/套装判断(L1581,需完整specialSeq常量映射)/觉醒(L2829)/法宝加成(L2863)/载具加成/增益计算/战斗力(L3031)。依赖 `套装判断`+`装备要求`(L1512) helper 与怪物对象扩展 |
