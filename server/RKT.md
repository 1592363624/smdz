...
|----------------------|-----------|--------------|------|
| 战斗入口（攻击/覅攻击pd/覅公jj/开始战斗/扫荡/停止战斗） | ~12 | 🔶 半 | `attack` 单向攻击已实现；`开始战斗/扫荡` 仅置标记，**战斗循环 driver 未实现**（见 §11.1） |
...
| 家园（家园/搬迁/命名/音乐/操作/前线/产出） | ~10 | 🔶 半 | 基础 CRUD ✅；`家园前线/产出` 子系统 ⬜（见 §11.3） |
| 副本（开启副本/刷新副本/副本清空） | ~5 | 🔶 半 | 副本清空 ✅；开启/刷新 ⬜ |
| 商店（使魔商店/兑换/设置购物） | ~6 | 🔶 半 | 商店刷新定时 ✅；兑换交互 ⬜ |
| 捕捉（捕捉/开始捕捉/停止捕捉） | ~4 | ⬜ 占位 | handler 已注册，内部未实现 |
...
- `生成前线 L5319`：函数 `generateFrontline` 已在 combat-system 建立，但**未从任何家园/建筑命令接线调用** → 属 §11.3 家园子系统一部分。
...
- **仍待补（下一轮，独立大项）**：载具部件级受击（损伤控制系统/贯穿抵抗/侵彻拆分，L3194-3288 全量）；天神降世(觉醒)在反击方怪物侧的加成；家园前线/产出（§11.3）；占位 handler 内部（§11.4）。
...
### §11.3 家园子系统（前线/产出）
- 原版 `使魔家园.ecode` + 生成前线 L5319 + 家园产出。当前 `家园` CRUD 已通，`家园前线/产出` 未接线。需独立评估工作量后单独立项。

### §11.4 占位 handler 内部实现
- `捕捉/炮击/福音书/安乐天使/呼叫/设置跟随` 等 handler 已注册但内部为占位或空白，需逐条从原版 ecode 复刻（建议在 §11.1 落地后按优先级排期）。
