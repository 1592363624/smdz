...
| 攻击目标 | L4021 | `combat-system` + `combat-state` | 🔶 | **基石层已建**：`combat-state.service.ts` 1:1 复刻 添加成就L678/取成就熟练度L719/置成就熟练度L850/标记要求L747/添加标记L778/增益要求L799/获得增益L1522/时间间隔要求L1008。护盾层/装甲层/生命层特效分支（激变星/强袭/坚韧护盾/盾逆/甲逆+木天蓼/日轮a/兰音破盾破甲成就/捕捉模式/麻醉衰减等）已分批接入；**吸血姬猩红真伤三池结转已于 2026-08-23 完成**（见 §12.11）。仅剩：兰音好感≥80 三池扣减显示文本差异（纯文本，数值正确） |
...
【复刻】calcDamage opts 新增 defenderMarkers/defenderBuffs（可选）；释放判定在分伤前，distributeDamageToPools 新增 vampireTrue 结转参数贯穿三层；applyDamageToMonster 内实现积累端（怪物侧）。weaponAttack 主目标/溅射目标/怪物反击玩家三处调用点已接线写回。🔶已知偏差：玩家作为受害者的积累端暂缺（怪物攻击玩家路径），"这个伤害不算攻击者"的总伤害归属差异保留。
