 ...
 *
 * 本 config 只匹配 14 个真实 DB 集成套件（清单见 jest.db-specs.cjs）。它们共享同一远程
 * smdz schema、在同一游戏地图上并发读写全局聚合状态，因此**绝不能并行**——务必用串行方式
 * 调用（见下），否则会互踩产生间歇性假失败。
 *
 * 调用方式（必须 --runInBand，且勿与 unit project 并行）：
 *   npx jest --config jest.db.config.js --runInBand      # 串行跑 14 个真实库套件
 *   npm run test:db                                      # 等价简写
 *
 * 判定全绿的唯一依据仍是 `npm test`（= jest --config jest.full.config.js --runInBand，
 * 一次串行跑满 unit+db 共 119 套件）。本 config 供需要单独回归真实库逻辑时使用。
 */
const { dbSpecs } = require('./jest.db-specs.cjs');

// 用 testMatch（glob，支持 <rootDir> 展开为绝对路径）精确匹配 14 个真实 DB 套件，
