 ...
 *
 * 为什么需要扩展名兜底：剪贴板截图、拖拽粘贴得到的 File 常常没有扩展名
 * （originalname 为 "image" / "blob" / 空串），若只取 extname(originalname) 落盘，
 * 文件名会丢后缀；静态服务只能按扩展名推断 Content-Type，无后缀会返回
...
/** MIME → 扩展名兜底映射（仅覆盖常见的图片类型，未命中则不补扩展名） */
