...
export class FamiliarSystemService {
  ...
  private async getPetOperationsHelp(userId: number): Promise<string> {
    ...
    // 原版 _主程序.ecode L8025：17 项编号临时输入替换，玩家直接发数字直达对应指令
    if (this.shortcutService?.setTempInput) {
      await this.shortcutService.setTempInput(
        userId,
        '1@宠物改名#2@宠物转让#3@全部跟随#4@全部停下#5@全部被动#6@全部主动#7@大召唤术#8@救助全部#9@全部挤奶#10@宠物驾驶#11@呼叫#12@宠物装备#13@宠物觉醒#14@宠物前往#15@宠物攻击#16@宠物喂食#17@宠物嗅探',
      );
    }
    ...
    const lines = [
      ...
      `主动:主动攻击`,
      `被动:挨打也不会反击`,
      `你可以“开始捕捉”来让你中意的怪物获得一个10分钟免疫生命伤害的buff`,
      `输入宠物名称即可，不用输入宠物的称号`,
      `————————`,
      `宠物自主行为(仅当前地图):`,
      `◆互动:触发几率【好感-100】%，单个宠物冷却10分钟，亲(攻击+15%)、蹭(闪避+15%)、抱(命中+15%)，时间可叠加`,
      `◆搜索:触发几率【(好感-100)÷4】%，获得随机物品，全部宠物共用冷却10分钟。`,
      `当宠物好感超过200时，会额外获得一件装备，并且优先触发搜索`,
      `宠物搜索到的建筑不受搜索数量加成，但是搜索到建筑时会额外多搜索一个物品`,
      `宠物嗅探可以让狩猎类宠物在当前地图寻找怪物，成功率与宠物战斗力有关，当前的狩猎类宠物有常春藤、各种狼、各种虎、巨齿鲨`,
    ];
