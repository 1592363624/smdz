 ...
 * 地图怪物自动攻击循环
 *
 * 复刻原版易语言 _主程序.ecode L200-535「覅攻击pd」的延时递归驱动：每回合随机一只
 * 存活怪物攻击全图玩家/召唤物，回合结束时若仍有目标，通过
 * 新建延时("覅攻击pd"+地图, "0", 群号, 4)（L504）自动续下一回合，直到 无目标
 * （"#没有目标"，战斗相关.ecode L47）或地图"活动"标记过期（L211）才停止。
 * 后台「延时执行」线程（后台运作.ecode L1532）每秒扫描待执行延时并按 (命令,qq) 去重
 * —— 同一地图同时最多一个待执行回合；玩家动作只负责拉起，不叠加并行循环。
 *
 * 触发点对齐原版 新建延时 调用位：攻击 _主程序.ecode L166（3秒）、采集 L11426（5秒）、
 * 传送/跃迁到达 L1761/1767/1790（5秒）、地精攻势 L2167（3秒）、
 * 使魔技能延时 使魔技能.ecode（5秒）。
 *
