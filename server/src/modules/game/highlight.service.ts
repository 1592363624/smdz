/**
 * 游戏高光时刻推送服务：把「任务达成 / 领取新任务 / 获得称号 / 等级提升」这类里程碑
 * 以**结构化事件**定向推送给当事玩家，供前端播放屏幕级高光动画。
 * - 与 ChatService 一样采用「网关初始化后注入 server 实例」的模式（见 ChatGateway.afterInit），
 *   不参与构造函数依赖注入，因此不会与 CommandModule / GameModule 之间形成循环依赖。
 * - 只按 `user:{userId}` 房间定向推送，不写库、不进公屏；推送失败（玩家离线 / socket 未就绪）
 *   静默降级，绝不影响主业务链路。
 * - 公屏文本仍照常产出，高光事件只是「额外的视觉增强」：前端既走结构化事件，也保留公屏文本兜底解析。
 */
...
export class GameHighlightService {
  ...
  /** 向指定玩家推送一条高光事件；userId 缺失/非法时静默忽略，at 由本方法补齐。 */
