 ...
 * 展示路径与战斗路径必须共用本纯函数：两侧各写一份会导致「面板看得到、打架打不出」。
 *
 * 原版语义（逐条对应）：
 *   L1442-1454  武器专属伤害属性缩放：37 全系×1.15 / 38 物×1.25 / 39 火×1.25 /
 *               40 冰×1.25 / 41 电×1.25
 *               （原版 39-41 写成「物 = 火×1.25」等同族笔误，会把武器物伤清零；
 *                此处按 effects.json 三条特效的描述实现，不跟随原版）
 *   L1458-1467  武器：z.特效=b；叠加加成(z.自带, 武器特效[b].加成)；
 *               z.冷却 += 武器特效[b].加成.冷却；
 *               攻击次数按「额外次数」原样追加（原版为 >=2 时追加 n-1；
 *               现数据的 攻击次数=1 在旧门槛下恒不生效，故与战斗侧统一改口径）；
 *               攻击文本非空时覆盖 z.攻击文本
...
export function applyEquipmentEffect(
  ...
  if (isWeapon) {
    ...
    switch (b) {
      case 37: // 天行者：全系×1.15
        ...
        break;
      case 38: // 引力弹头：[武器的物理伤害百分比x1.25]
        p.phys *= 1.25;
        break;
      case 39: // 龙之吐息：[武器的火焰伤害百分比x1.25]
        p.fire *= 1.25;
        break;
      case 40: // 灵魂之息：[武器的冰冻伤害百分比x1.25]
        p.ice *= 1.25;
        break;
      case 41: // 球状闪电：[武器的闪电伤害百分比x1.25]
        p.elec *= 1.25;
        break;
      default:
        break;
  ...
  if (isWeapon) {
    ...
    // 「攻击次数」按**额外攻击次数**取用：数据里 航空母舰(舰队已抵达) 的加成是 1，
    // 原版的 `>=2 才追加 n-1` 门槛会让它恒为 0（键写了但永远不生效）。
    // 与战斗侧 weaponAttackInner 的口径保持一致（同为「额外 N 次」）。
    const attackCount = toNum(bonus['攻击次数']);
    if (attackCount >= 1) {
      target.attackBonus['攻击次数'] = (target.attackBonus['攻击次数'] || 0) + attackCount;
    }
