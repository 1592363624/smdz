/**
 * 游戏动态数据种子导入（来源 prisma/data/*.json）：只初始化需要持久化的动态数据
 * —— GameMap 运行时状态（spawnMonsters/resources2/markers 等）与 GameVehicle 载具实例。
 * 固定配置（怪物/物品/装备/任务/商店…）不入库，运行时由 StaticDataService 直接读 JSON。
 *
 ...
 * DB 只存 name（唯一键）、mapIndex（排序）与运行时动态字段：
 ...
 * 首次导入时半动态字段取 JSON 初始值；后续更新保留 DB 中的运行时状态。
 */
async function seedMapDynamicFields(): Promise<number> {
  ...
  for (const row of maps) {
    ...
    try {
      // upsert：create 写入 name/mapIndex + 半动态字段初始值；update 只同步 name/mapIndex 与半动态字段
      await prisma.gameMap.upsert({
        ...
        // update 同步"以 JSON 为准"的固定配置字段（半动态列），保留纯运行时状态
        // （spawnMonsters/tempMonsters/summons/markers/markers2）：改 maps.json 后存量库能同步生效。
        update: {
          name,
          mapIndex: (row as any).mapIndex ?? 0,
          ...semiDynamicFields,
...
async function importDynamicData() {
  ...
  // 2. 载具（玩家拥有的载具 owner/driver/currentHp 在 DB 持久化）；
  //    vehicles.json 含载具/部件模板，仅导入 gameVehicle 作初始化模板。
